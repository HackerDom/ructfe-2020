...
def signup():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        if db.session.query(exists().where(User.username == request.form['username'])).scalar():
            ...
            login_user(user)
            flash(Markup(
                f'Your autogenerated password is <strong>{user.generate_password()}</strong> <br/>'
...
def sign():
    ...
    if not document.is_public:
        flash(Markup(
            f'This document\'s autogenerated password is <strong>{document.generate_password()}</strong> <br/>'
            f'Store it somewhere safe!'))
